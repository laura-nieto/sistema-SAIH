<div class="w-full">
    <div class="py-12 flex-1 px-2 md:px-10">
        <x-slot name="header">
            {{ __('Lista de Departamento de Colaboradores') }}
        </x-slot>
        @if(session('success'))
            <x-success>{{ session('success') }}</x-success>
        @endif
        <x-seccion-white>
            @can('admin.departamento_colaborador.create')
            ...
            @endcan
            @if ($modal)
                @include('livewire.depto-colaborador.modal-departamento-colaborador')
            @endif
                            ...
                            @foreach($dptos as $dpto)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $dpto->nombre }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex justify-evenly">
                                            @can('admin.departamento_colaborador.edit')
                                                <button wire:click='editar({{$dpto->id}})'
                                            ...
                                            @endcan
                                            @can('admin.departamento_colaborador.destroy')
                                                @if($dpto->deleted_at == null)
                                                    <button wire:click='abrirModalDelete({{$dpto->id}})'
