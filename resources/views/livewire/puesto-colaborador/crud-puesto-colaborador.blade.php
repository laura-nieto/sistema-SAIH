<div class="w-full">
    <div class="py-12 flex-1 px-2 md:px-10">
        <x-slot name="header">
            {{ __('Lista de Puestos de Colaboradores') }}
        </x-slot>
        @if(session('success'))
            <x-success>{{ session('success') }}</x-success>
        @endif
        <x-seccion-white>
            @can('admin.puesto_colaborador.create')
            ...
            @endcan
            @if ($modal)
                @include('livewire.puesto-colaborador.modal-puesto-colaborador')
            @endif
                            ...
                            @foreach($puestos as $puesto)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $puesto->nombre }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex justify-evenly">
                                            @can('admin.puesto_colaborador.edit')
                                            ...
                                            @endcan
                                            @can('admin.puesto_colaborador.destroy')
                                                @if($puesto->deleted_at == null)                            
                                                    <button wire:click='abrirModalDelete({{$puesto->id}})'
