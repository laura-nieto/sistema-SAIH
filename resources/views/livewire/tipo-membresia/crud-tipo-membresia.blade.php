<div class="w-full">
    <div class="py-12 flex-1 px-2 md:px-10">
        <x-slot name="header">
            {{ __('Lista de Tipos de Membresia') }}
        </x-slot>
        @if(session('success'))
            <x-success>{{ session('success') }}</x-success>
        @endif
        <x-seccion-white>
            @can('admin.tipo_membresia.create')
            ...
            @endcan
            @if ($modal)
                @include('livewire.tipo-membresia.modal-tipo-membresia')
            @endif
            <div class="overflow-hidden sm:px-6 lg:px-8">
                ...
                <table class="text-black min-w-full divide-y divide-gray-200 sm:rounded-lg">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @if(!$membresias->count())
                            <tr>
                                <td class="px-6 py-4 border-b-2" colspan="5">No existen membresías</td>
                            </tr>
                        @else
                            @foreach($membresias as $membresia)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $membresia->nombre }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex justify-evenly">
                                            @can('admin.tipo_membresia.edit')
                                                <button wire:click='editar({{$membresia->id}})'
                                            ...
                                            @endcan
                                            @can('admin.tipo_membresia.destroy')
                                                @if($membresia->deleted_at == null)
                                                    <button wire:click='abrirModalDelete({{$membresia->id}})'
